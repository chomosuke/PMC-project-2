...
\begin{algorithmic}[1]
	\Function{Simulate}{points, velocities}
		...
		\State $BHTree \gets \Call{BarnesHutTree}{points}$
		\For{$i \gets 1$ \textbf{to} $m$}
...
\caption{}
\begin{algorithmic}[1]
	\Function{BarnesHutTree}{BHTree, points}
...
\begin{algorithmic}[1]
	\Function{GetAcceleration}{point, BHTreeNode}
		...
		\Call{CenterOfMass}{BHTreeNode}}} < \theta \lor \Call{NumChildren}{BHTreeNode} = 0$}
			\State $acceleration \gets \Call{AccelerationFromCOM}{point,
			\Call{CenterOfMass}{BHTreeNode}, \Call{Mass}{BHTreeNode}}$
		\Else
			\For{$child \gets $ each child of $BHTreeNode$}
